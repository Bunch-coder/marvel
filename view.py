...
class SearchView:
    ...
    def setup_search_frame(self):
        ...
        self.username_label = tk.Label(self.search_frame, text="Email:", bg=BG_COLOR, fg=TEXT_COLOR, font=LABEL_FONT)
        self.username_label.grid(row=3, column=0, padx=(10, 10), pady=(20, 0), sticky=tk.W)

        self.username_entry = tk.Entry(self.search_frame, width=20, bg=SECONDARY_COLOR, fg=PRIMARY_COLOR,
                                       font=LABEL_FONT)
        ...
        self.password_entry = tk.Entry(self.search_frame, width=20, bg=SECONDARY_COLOR, fg=PRIMARY_COLOR,
    ...
    def login(self):
        ...
        if success:
            ...
            welcome_label = tk.Label(self.search_frame, text=f"Welcome, {username}!", bg=BG_COLOR, fg=TEXT_COLOR,
                                     font=LABEL_FONT)
    ...
    def open_favorite_window(self):
        ...
        self.favorite_window.geometry("800x300")
        ...
        if favorite_comics:
            for comic in favorite_comics:
                ...
                title_label = tk.Label(comic_frame, text=comic['comic_title'], bg=BG_COLOR, fg=TEXT_COLOR,
                                       font=LABEL_FONT)
                title_label.pack(side=tk.LEFT, anchor=tk.W)

                remove_button = tk.Button(comic_frame, text="Remove", bg=ACCENT_COLOR, fg=SECONDARY_COLOR,
                                          font=SMALL_FONT,
                                          command=lambda comic_id=comic['id']: self.remove_from_favorites(comic_id))
            ...
            empty_label = tk.Label(favorite_frame, text="No favorite comics yet.", bg=BG_COLOR, fg=TEXT_COLOR,
                                   font=LABEL_FONT)
            empty_label.pack()

        favorite_frame.bind("<Configure>",
                            lambda e: favorite_canvas.configure(scrollregion=favorite_canvas.bbox("all")))

        self.refresh_favorite_window.bind("<Destroy>", self.on_favorite_window_close)

        self.favorite_window.mainloop()

    def on_favorite_window_close(self, event):
        favorite_canvas = self.favorite_window.nametowidget("favorite_canvas")
        favorite_canvas.unbind_all("<MouseWheel>")
    ...
    def remove_from_favorites(self, comic_id):
        ...
        if success:
            ...
            self.refresh_favorite_window()
        else:
            messagebox.showerror("Error", "Failed to remove comic from favorites.")

    def refresh_favorite_window(self):
        # Clear the existing contents of the favorites window
        for widget in self.root.winfo_children():
            if isinstance(widget, tk.Toplevel) and widget.title() == "Favorite List":
                widget.destroy()

        # Open a new favorites window with the updated list of comics
    ...
    def login_user(self, username, password):
        ...
        cursor = self.db.execute_query(login_query, (username, password))
    ...
    def display_comics_list(self, comics_list):
        ...
        self.comics_window = tk.Toplevel(self.root)
        self.comics_window.title("Comics List")
        self.comics_window.configure(bg=BG_COLOR, pady=5)
        self.comics_window.geometry('800x600')

        # create a Canvas widget to show the comics list with custom scrolling
        comics_canvas = tk.Canvas(self.comics_window, bg=BG_COLOR, highlightthickness=0, name="comics_canvas")
        ...
        pagination_frame = tk.Frame(self.comics_window, bg=BG_COLOR)
        pagination_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=10)
        ...
        self.comics_window.bind("<Destroy>", self.on_comics_window_close)
        ...
        self.comics_window.mainloop()

    def on_comics_window_close(self, event):
        comics_canvas = self.comics_window.nametowidget("comics_canvas")
        comics_canvas.unbind_all("<MouseWheel>")
        self.results_canvas.bind_all("<MouseWheel>", self.on_mousewheel)
